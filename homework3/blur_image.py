...
def read_grid(file_path):
    ...
    grid_list = []
    for line in reading:
        inner_list = csv_line_to_list(line)
...
def get_pixel_at(pixel_grid, i, j):
    ...
    """
    if i < 0 or j < 0 or len(pixel_grid) <= i or len(pixel_grid[0]) <= j:
        return 0
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    pixel_sum = 0
    for k in range(3):
        pixel_sum += get_pixel_at(pixel_grid, i - 1 + k, j - 1)
        pixel_sum += get_pixel_at(pixel_grid, i - 1 + k, j)
        pixel_sum += get_pixel_at(pixel_grid, i - 1 + k, j + 1)
...
def blur(pixel_grid):
    ...
    for r in range(len(pixel_grid)):
        ...
        for c in range(len(pixel_grid[0])):
            row.append(average_of_surrounding(pixel_grid, r, c))
        pixel_grid_copy.append(row)
    return pixel_grid_copy
